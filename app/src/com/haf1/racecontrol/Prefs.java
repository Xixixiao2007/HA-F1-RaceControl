...
public class Prefs {
     ...
     * 实测（一个比赛周末 697 条消息）：
     *   隐藏 253/697 = **36.3%**
     * v2.0.6 之前是 382/697 = 54.8% —— 那时把解除信号（CLEAR）也当噪音藏了，
     * 一共 129 条。现在 CLEAR 全部显示：藏掉它会让黄旗「莫名其妙就结束」，
     * 看不到是哪个区段被解除（用户要求）。蓝旗仍然是噪音。
     *
     ...
     * ## 明确不隐藏的
     * - 黑白旗警告（用户要求）
     * - **解除信号**（`CLEAR IN TRACK SECTOR n` / `TRACK CLEAR`）（用户要求：
     *   藏掉它会让黄旗莫名其妙就结束，看不到是哪个区段被解除）
